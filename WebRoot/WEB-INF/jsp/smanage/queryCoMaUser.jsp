...
function deleteRow(user_id)
{
	...
	{
		if(table.rows[i].cells[0].innerText==user_id)
		{
			table.deleteRow(i);
			break;
		}
...
<tbody>
<c:forEach items="${coma_list}" var="user">
<tr>
	<th>${user.user_id}</th>
	<th>${user.password}</th>
	<th><a href="editCoMaUser.action?user_id=${user.user_id}">修改</a >/<a href="javaScript:delete_remind(${user.user_id})">删除</a></th>
	</tr>
</c:forEach>
</tbody>
</table>
...
function delete_remind(user_id)
{
	if(confirm('是否删除?'))
	{
		createDeleteRequest("deleteUser.action?user_id="+user_id);
	}	
}
</script>
	</body>
